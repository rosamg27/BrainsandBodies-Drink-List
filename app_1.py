...
def staff_view(conn):
    ...
    with tab1:
        ...
        cust_rows = conn.execute(
            "SELECT name, credits FROM customers ORDER BY name"
        ).fetchall()
        if cust_rows:
            st.dataframe(
                pd.DataFrame(
                    [{"Name": n, "Guthaben": c} for n, c in cust_rows]
                ),
...
def main_router():
    ...
    if "staff" in params:
        ...
        st.subheader("Wie heißt du? / What's your name?")
        name_input = st.text_input(
            "Bitte gib deinen Namen ein / Please enter your name"
        )

        if name_input:
            name_by_lower = {n.strip().lower(): cid for cid, n in rows}
            match = name_by_lower.get(name_input.strip().lower())
            if match:
                st.session_state.selected_uid = match
                st.rerun()
            else:
                st.error(
                    "Name nicht gefunden. Bitte beim Personal melden.\n\n"
                    "Name not found. Please contact staff."
                )
